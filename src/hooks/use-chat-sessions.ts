 ...
 * 
 * API Integration:
 * - เชื่อมต่อกับ /api/chat_06_summary/session
...
export function useChatSessions(userId?: string) {
  ...
  const fetchSessions = async () => {
    ...
    try {
       ...
       * 
       * API Endpoint: /api/chat_06_summary/session
       * Query Parameter: userId (encoded สำหรับความปลอดภัย)
       * 
       * Expected Response:
       * - sessions: array ของ ChatSession objects
       */
      const response = await fetch(`/api/chat_06_summary/session?userId=${encodeURIComponent(userId)}`)
  ...
  const createSession = async (title?: string) => {
    ...
    try {
       ...
       */
      const response = await fetch('/api/chat_06_summary/session', {
  ...
  const deleteSession = async (sessionId: string) => {
    ...
    try {
      const response = await fetch(`/api/chat_06_summary/session?sessionId=${sessionId}`, {
        method: 'DELETE',
      })
